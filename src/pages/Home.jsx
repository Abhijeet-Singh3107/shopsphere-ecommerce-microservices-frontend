...
export default function Home() {
  ...
  return (
    <div className="bg-gray-50">
      {/* Hero */}
      <section className="relative overflow-hidden bg-gradient-to-br from-sky-700 via-sky-600 to-cyan-500 py-24 text-white">
        <div className="pointer-events-none absolute -top-20 -right-20 h-80 w-80 rounded-full bg-white/10 blur-3xl" />
        <div className="pointer-events-none absolute bottom-0 left-0 h-60 w-60 rounded-full bg-cyan-400/20 blur-2xl" />
        <div className="relative mx-auto max-w-4xl px-4 text-center">
          ...
          <h1 className="mb-5 text-5xl font-extrabold tracking-tight sm:text-6xl">
            Shop smarter.<br />
            <span className="text-sky-200">Live better.</span>
          </h1>
          <p className="mb-8 mx-auto max-w-xl text-lg text-sky-100">
            Thousands of products, unbeatable prices, delivered to your door.
          </p>
          <div className="flex flex-wrap justify-center gap-3">
            <Link to="/products" className="rounded-full bg-white px-8 py-3 font-semibold text-sky-700 shadow-lg transition-all hover:bg-sky-50 hover:shadow-xl">
              Shop Now
            </Link>
      ...
      <section className="mx-auto max-w-7xl px-4 py-14">
        <div className="mb-8 flex items-end justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-sky-600">Handpicked for you</p>
            <h2 className="mt-1 text-3xl font-bold text-gray-900">Featured Products</h2>
          </div>
          <Link to="/products" className="text-sm font-semibold text-sky-600 hover:text-sky-800">
            View all →
          </Link>
        </div>
        {loading ? (
      ...
      <section className="mx-auto max-w-7xl px-4 pb-14">
        <div className="flex flex-col items-center justify-between gap-6 rounded-2xl bg-sky-600 px-8 py-10 text-white sm:flex-row">
          <div>
            <h3 className="text-2xl font-bold">Ready to explore more?</h3>
            <p className="mt-1 text-sky-200">Browse our full catalog and find exactly what you need.</p>
          </div>
          <Link to="/products" className="shrink-0 rounded-full bg-white px-6 py-3 font-semibold text-sky-700 hover:bg-sky-50 transition-colors">
            Browse All Products
          </Link>
        </div>
      </section>
