...
export default function Navbar() {
  ...
  const linkCls = 'text-sm font-medium text-gray-600 hover:text-indigo-600 transition-colors';
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between gap-4">
          ...
          <Link to="/" className="flex items-center gap-2 shrink-0">
            <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-indigo-600 text-white font-bold text-sm">S</span>
            ...
            <form onSubmit={handleSearch} className="hidden flex-1 max-w-sm sm:flex">
              <div className="relative w-full">
                <input
                  ...
                  className="w-full rounded-full border border-gray-200 bg-gray-50 py-2 pl-4 pr-10 text-sm focus:border-indigo-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-200"
                />
          ...
          <div className="hidden items-center gap-5 sm:flex">
            {!role && (
              <>
                ...
                <Link to="/register" className="rounded-full bg-indigo-600 px-4 py-1.5 text-sm font-semibold text-white hover:bg-indigo-700 transition-colors">
                  Register
                </Link>
              </>
            )}
          ...
          {/* Mobile menu button */}
          <button onClick={() => setMenuOpen((o) => !o)} className="sm:hidden p-2 text-gray-500 hover:text-indigo-600">
...
function MobileLink({ to, onClick, children }) {
  return (
    <Link to={to} onClick={onClick} className="block py-2 text-sm font-medium text-gray-700 hover:text-indigo-600">
      {children}
    </Link>
  );
}
